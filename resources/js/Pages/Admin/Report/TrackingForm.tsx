...
export default function TrackingForm({ tracking }: PageProps & { tracking?: ReportTrackingType }) {
    ...
    const handleSubmit: FormEventHandler = (e) => {
        e.preventDefault()
        if (tracking) {
            put(route('admin.report.tracking.update', { reportId, tracking: tracking.id }), {
                onError: (e) => {
                    swalError(e.error)
                },
                onSuccess: () => {
        ...
        post(route('admin.report.tracking.store', { reportId }), {
            onError: (e) => {
                swalError(e.error)
            },
            onSuccess: () => {
                    ...
                    <div className="card">
                        <div className="card-body">
                            ...
                            <form className="forms-sample" onSubmit={handleSubmit}>
                                ...
                                <button type="button" className="btn btn-light" onClick={() => router.visit(route('admin.report.show', { report: reportId }))}>Cancel</button>
                            </form>
                        </div>
                    </div>
                </div>
